...
;; subforms.
;;
;; Ignore exprs which are not list. LazySeqs are generated by logic/run* so we
;; shouldn't skip those because they may need to be simplified more than once.
;; Example with 2 simplification:
;; (if true (+ 1 a) nil) -> (when true (+ 1 a)) -> (when true (inc a)
(defn simplify-one [expr rules]
  (if-not (or (list? expr)
              (= clojure.lang.LazySeq (type expr)))
    expr
